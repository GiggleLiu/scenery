...
// Adapter for the former `projector(view)` closure over scenery's
...
#let sc = crystal-scene(nacl, view: (azimuth: 30deg, elevation: 15deg))
...
#let custom = crystal-scene(nacl, colors: (Na: custom-blue,))
...
#assert(sc.bbox.min.at(0) < sc.bbox.max.at(0))
#assert(sc.screen-bbox.at(0) < sc.screen-bbox.at(2))
// polyhedra path
#let sto = structure(
  spacegroup: 221, lattice: (a: 3.905),
  sites: ((element: "Sr", wyckoff: "a"), (element: "Ti", wyckoff: "b"), (element: "O", wyckoff: "c")),
)
#let sc2 = crystal-scene(sto, view: (azimuth: 30deg, elevation: 15deg),
