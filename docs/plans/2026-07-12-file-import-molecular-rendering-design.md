...
- **Auto bond detection in Rust at import time.** The distance rule over all atom pairs is O(N²) and, for large imported molecules, dominates compile time long before depth-sorting does. The parser runs a spatial-hash neighbor search (covalent radii compiled into the crate, same 1.15×/0.4 Å rule) and fills the record's `bonds` field. The Typst rule-based path in `geometry.typ` remains for hand-built structures and custom bond rules, which override imported bonds.
...
2. **Periodic formats** — extended-xyz, POSCAR, CIF (op-loop application + spacegroup-lookup sub-paths). Reuse crystal lattice+basis and Typst symmetry expansion. Rust auto bond detection lands here (imports need it; Stage 1's `.xyz` demo may use the Typst rule as a stopgap).
...
4. **Scale & translucent correctness** — `scenery-engine` crate: projection/depth-sort accelerator + BSP splitting for translucent faces, shipped in `scenery/`. Renders large crystal supercells; fixes intersecting-translucent-polyhedra ordering.
