// User-facing figure layer for brillouin: the textbook Brillouin-zone drawing
...
// cell (no transform); a wyckoff structure carries only its conventional
// `vectors` and no Bravais symbol, so its k-path cannot be drawn (see below).

#import "@preview/scenery:0.1.0" as scenery
#import "reciprocal.typ": reciprocal-vectors, params-to-vectors, from-wyckoff
...
#let _primitive-vectors(bravais, params) = {
  ...
  if bravais in ("cP", "tP", "oP", "mP", "hP", "aP") {
    ...
    panic("bz-figure: mC (base-centered monoclinic) has no closed-form primitive"
      + " cell here; pass the primitive lattice vectors directly as `lattice`")
  } else {
    panic("bz-figure: unknown Bravais symbol '" + bravais + "'")
  }
}
...
// a wyckoff structure (has a `vectors` field; conventional cell, no centering
...
#let _recip(lattice, bravais) = {
  if type(lattice) == dictionary and "vectors" in lattice {
    ...
    panic("bz-figure: `lattice` must be a parameter dict, three direct vectors, "
      + "or a wyckoff structure; got " + repr(type(lattice)))
  }
}

// Parameters for kpath-data: a params dict is used directly; anything else needs
// an explicit `params:` (direct vectors / wyckoff structures carry no a,b,c,..).
#let _params-for-kpath(lattice, params) = {
  if params != auto { params }
    ...
    panic("bz-figure: drawing a k-path needs cell parameters — pass a parameter "
      + "dict as `lattice`, or give `params: (a: .., ..)` explicitly (direct "
      + "vectors and wyckoff structures carry no a,b,c,angles)")
  }
}
...
// Build the pure-data scene shared by bz-figure (renders to content) and
// bz-group (emits cetz commands). `params` is threaded through for the k-path.
#let _scene(lattice, bravais, view, kpath, klabels, highlight, path, params) = {
  ...
  if bz == none {
    panic("bz-figure: degenerate lattice — the reciprocal vectors are coplanar, "
  ...
  if highlight.len() > 0 and not has-symbol {
    panic("bz-figure: `highlight:` names high-symmetry points, which are only "
  ...
  if (kpath or highlight.len() > 0) and has-symbol {
    ...
    for h in highlight {
      if norm(h) not in data.points {
        panic("bz-figure: highlight point '" + h + "' is not in the " + bravais
...
///   vectors, or a wyckoff structure value (its conventional `vectors` are read;
...
#let bz-figure(
  ...
  let scene = _scene(lattice, bravais, view, kpath, klabels, highlight, path, params)
...
/// The same scene as `bz-figure`, emitted as raw cetz draw commands for
/// composition inside an existing `cetz.canvas` (see `scenery.scene-group`).
///
/// Takes the same arguments as `bz-figure` but `scale:` (canvas units per scene
/// unit) instead of `width:`.
/// -> content
#let bz-group(
  lattice,
  bravais: auto,
...
#let band-axis(bravais, params, points, samples: 24) = {
  ...
  for nm in seq {
    if nm not in data.points {
      panic("band-axis: point '" + nm + "' is not in the " + bravais
        + " k-point set " + repr(data.points.keys()))
    }
  }
...
#let band-panel(
  ...
  assert(bands.len() > 0, message: "band-panel: `bands` is empty")
  assert(kd.len() > 1, message: "band-panel: need at least two k-distance samples")
