...
//
// Pure functions on scenery's linalg — cetz-free, no wyckoff dependency.
#import "@preview/scenery:0.1.0": vcross, vdot, vscale

// --- params -> direct vectors -------------------------------------------------
//
// This reimplements wyckoff's params->vectors convention (wyckoff/src/lattice.typ,
// `lattice-vectors`) so that brillouin's reciprocal vectors are CONSISTENT with a
// wyckoff structure's `vectors` field WITHOUT importing wyckoff (soft dependency).
// The basis orientation MUST match wyckoff exactly:
//   a1 = (a, 0, 0)                          (a along x)
//   a2 = (b cosγ, b sinγ, 0)                (b in the xy-plane)
//   a3 = (c cosβ, c (cosα - cosβ cosγ)/sinγ, c z)
// If wyckoff ever changes its orientation, this block must be updated in lockstep;
...
// mirroring wyckoff's lattice-params defaults: b and c default to a, angles to 90.
#let _fill-params(p) = {
  let a = _deg(p.at("a"))
  (
    a: a,
...
///   of three 3-vectors in Å — or a crystal-system parameter dict with keys
///   `a`, `b`, `c` (Å) and `alpha`, `beta`, `gamma` (angles or degrees). Missing
///   lengths default to `a`; missing angles default to 90°, matching wyckoff.
/// -> array
/// Returns `(b1, b2, b3)`, the three reciprocal vectors in Å⁻¹, with
/// b_i = 2π (a_j × a_k) / (a_1 · (a_2 × a_3)) (cyclic).
#let reciprocal-vectors(direct) = {
  let (a1, a2, a3) = if _is-vectors(direct) { direct } else { params-to-vectors(direct) }
...
/// Reciprocal vectors from a wyckoff structure value (soft dependency).
///
/// Reads the `vectors` field off `structure` by plain data access — brillouin
/// never imports wyckoff. Field contract: a wyckoff structure carries `vectors`
/// as three 3-vectors (its direct lattice, in Å; see wyckoff/src/structure.typ).
///
/// - structure (dictionary): a wyckoff structure value with a `vectors` field.
/// -> array
#let from-wyckoff(structure) = reciprocal-vectors(structure.vectors)
