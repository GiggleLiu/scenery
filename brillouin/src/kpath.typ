...
#let _validate(bravais, q) = {
  ...
  let ang90 = (n, v) => if not _approx(v, 90.0, atol) {
    panic(bravais + ": angle " + n + " must be 90 deg (got " + repr(v) + ")")
  }
  let eqlen = (n, v, ref) => if not _approx(v, ref, ltol) {
    panic(bravais + ": length " + n + " must equal " + repr(ref) + " (got " + repr(v) + ")")
  }
  if bravais in ("cP", "cF", "cI") {
    ...
    if not _approx(ga, 120.0, atol) {
      panic(bravais + ": gamma must be 120 deg (hexagonal setting; got " + repr(ga) + ")")
    }
    eqlen("b", b, a)
  } else if bravais in ("mP", "mC") {
    ang90("alpha", al); ang90("gamma", ga)
    if _approx(be, 90.0, atol) {
      panic(bravais + ": beta must differ from 90 deg for a monoclinic cell")
    }
  } else if bravais == "aP" {
    // no constraint; reduced-cell requirement checked during selection
  } else {
    panic("unknown Bravais symbol '" + bravais + "' (expected one of " + repr(_bases) + ")")
  }
}
...
#let _variant(bravais, q) = {
  ...
  if bravais == "cP" { "cP2" }
    ...
    if cka <= tol and ckb <= tol and ckg <= tol { "aP2" }
      ...
      panic("aP: cell is not reduced (reciprocal cell neither all-acute nor "
...
#let _table(variant, q) = {
  ...
  if variant in ("cP1", "cP2") {
    ...
    panic("kpath: variant '" + variant + "' not implemented")
  }
}
